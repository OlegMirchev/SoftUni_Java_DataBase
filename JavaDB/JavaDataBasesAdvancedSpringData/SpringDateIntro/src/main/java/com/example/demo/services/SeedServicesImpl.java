...
public class SeedServicesImpl implements SeedServices {
    ...
    @Autowired
    public SeedServicesImpl(AuthorRepository authorRepository, CategoryRepository categoryRepository, BookRepository bookRepository, AuthorService authorService, CategoryService categoryService) {
    ...
    public void seedBooks() throws IOException {
        Files.readAllLines(Path.of(RESOURCE_PATH + BOOKS_FILE_NAME))
                .stream().filter(a -> !a.isBlank())
                .map(this::getBookItems)
                .forEach(bookRepository::save);
    }

    private Book getBookItems(String line) {
        String[] bookItems = line.split("\\s+");
        EditionType editionType = EditionType.values()[Integer.parseInt(bookItems[0])];
        LocalDate releaseDate = LocalDate.parse(bookItems[1], DateTimeFormatter.ofPattern("d/M/yyyy"));
        int copies = Integer.parseInt(bookItems[2]);
        BigDecimal price = new BigDecimal(bookItems[3]);
        AgeRestriction ageRestriction = AgeRestriction.values()[Integer.parseInt(bookItems[4])];
        String title = Arrays.stream(bookItems).skip(5).collect(Collectors.joining(" "));
